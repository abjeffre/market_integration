...
sim_dol_ngoods <- function( 
  ...
  #build inputs
  yeild <- rep (yeild, ngoods)
  skew = rep(skew, ngoods)
  gamma = rep(gamma/ngoods ,ngoods)
  
  #Initalize Arrays
  p <- array(0, c(n, ngoods, ngen))                                  # Proption of lifetime effort dedicated to producing each good
  prices <- array(0, c(n, ngoods, ngen))                             # An inhereted price vector from Parents
  learn <- array(0, c(n, ngoods, ngen))                              # Time spent learning - optimized under parameter constraints
  do <- array(0, c(n, ngoods, ngen))                                 # Time spent producing  (1 - learning time)
  produce <- array <- array(0, c(n, ngoods, ngen))                   # Goods produced
  inv <- array(0, c(n, ngoods, ngen))                                # Inventory
  pre_sale_inv <- array(0, c(n, ngoods, ngen))                       # Inventory - mostly used to check for errors
  optima <- array(0, c(n, ngoods, ngen))                             # Optimal amount of good to satisify their utility function  - mashallian demand 
  need <- array(0, c(n, ngoods, ngen))                               # The diffrence between the Optima and current Inventory
  sell <-  array(0, c(n, ngoods, ngen))                              # A matrix of sales tranctions 
  buy <-  array(0, c(n, ngoods, ngen))                               # A matrix of purchases
  visits <- matrix(0, nrow =n, ncol = ngen)                          # Total number of visits to others - attempts to buy
  visited <- matrix(0, nrow = n, ncol = ngen)                        # Total number of times visted by others - attempts to sell
  failed_sell <-  array(0, c(n, ngoods, ngen))                       # A matrix of recorded failures to transact when selling 
  failed_buy <-  array(0, c(n, ngoods, ngen))                        # A matrix of purchases failures to transact when buying
  prices_r <- array(NA, c(n, ngoods, ngen))                          # If a learner what are their new prices
 
  
  columns <- c("gid","id", "p", "trade_type", "share_type", "price_learn", "utility")
  ...
  #Set up trade_types 1 type one does not trade and type 2 does
  agents[, "price_learn", 1] <- sample(1:2, n, replace = T)
  ...
  for ( t in 1:(ngen) ) {
                  ...
                  if(need[i, g, t] > 0 & need[j, g, t] < 0){
                    ...
                    if(maxAvailibe >= need[i, g, t]) {  #the reciever evaluates their stock and sees if they can sell to the bidders total demand
                      ...
                      if(sum(proposal_sell_quant*prices[j, , t]) < (proposal_buy_quant*prices[j, g, t])) {
                        failed_sell[j,g,t] <- failed_sell[j, g, t] +  1                                           #record failed transaction for reciever
                        failed_failed[j,g,t] <- failed_buy[j, g, t] + 1
    ...
    ###########################################################
    ##### INDIVIDUAL ASSESSES IF THIER BID IS HIGH OR LOW #####
    
    #The indvidual looks at the number of successful trades to unsuccessful trades that they did. 
    #If the number of successful trades is high, they raise their price.
    #If the number of successful trades is low, they lower their price.
    
    if(agents[i,"price_learn",t] == 1){
      buy_attempts <- rowSums(buy[i,g,t] != 0) + failed_buy[i,g,t]
      sell_attempts <- rowSums(sell[i,g,t] != 0) + failed_sell[i,g,t]
      for{g in g:ngoods}
      
      #more succeses?
      if (rowSums(buy[i,g,t] != 0) + rowSums(buy[i,g,t] != 0) >  (failed_buy[i,g,t] + failed_sell[i,g,t])) prices_r[i,g,t] <- prices[i,g,t] + rexp(1,1)         
      #less success?
      if (rowSums(buy[i,g,t] != 0) + rowSums(buy[i,g,t] != 0) <  (failed_buy[i,g,t] + failed_sell[i,g,t])) prices_r[i,g,t] <- prices[i,g,t] + rexp(1,1) 
      #less success?
      if (rowSums(buy[i,g,t] != 0) + rowSums(buy[i,g,t] != 0) <  (failed_buy[i,g,t] + failed_sell[i,g,t])) prices_r[i,g,t] <- prices[i,g,t]
    ...
    if(agents[i,"price_learn",t] == 2) babies_prices <- prices_r[pid,,t]
    babies_p <- p[pid,, t]
    babies_share_type <- agents[pid, "share_type", t]
    babies_trade_type <- agents[pid, "trade_type", t]
    babies_price_learn_type <- agents[pid, "price_learn_type", t]
    ...
    for ( i in 1:n) {
      ...
      if ( runif(1) < mu ) babies_price_learn_type[i] <- ifelse(babies_price_learn_type[i]==1, 2, 1) 
    ...
    if(t< ngen){
      ...
      agents[,"price_learn",t+1] <- babies_price_learn_type
    }
    
  }#end time
  output[sim_num] <- list(agents = agents,
                 p = p,
                 prices = prices,
                 produce = produce,
                 do = do,
...
#   mc.cores=10)
# 
# saveRDS(results_dol, file = "sim_spec_trade_sweep")
# 
# 
# 
# #############################
